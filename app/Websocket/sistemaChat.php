...
class sistemaChat implements MessageComponentInterface {
    ...
    private function conversasDoUsuario($id_user) {
        try {
            // Use Eloquent para obter as conversas do usuário
            $resultConvs = Conversa::whereHas('participantes', function ($query) use ($id_user) {
    ...
    private function verificaçãoUsuarioConversa($conversationId, $from) {  //OK
        try {
            ...
            $stmt = $conn->prepare("SELECT COUNT(*) AS autorizado FROM participantes_conversa WHERE id_conversa = :conversationId AND id_user = :userId");
    ...
    private function participanteConversa($conversationId, $from) { // OK
        try {
            ...
            // Consulte a tabela 'participantes_conversa' para obter a lista de participantes.
            $userId = $this->pegarIdUsusarioConectado($from);

            $dbConnection = new dbConnection();
            $conn = $dbConnection->getConnect();

            $query = "SELECT id_user FROM participantes_conversa WHERE id_conversa = :conversationId AND id_user != :id_user LIMIT 1";
