...
async function carregarMsg(idConversa) {
    try {
        const dados = await fetch(`/carregar-mensagens?id_conversa=${idConversa}&offset=${offset}`);
        const resposta = await dados.json();

        if (resposta.status) {
            resposta.dados.forEach(item => {
                const mensagem = `${item.nome}: ${item.mensagem_text} <br>`;
                mensagemChat.insertAdjacentHTML('afterbegin', userId == item.id_user ? mensagem : mensagem);
            });

            offset += resposta.qtd_msg;
